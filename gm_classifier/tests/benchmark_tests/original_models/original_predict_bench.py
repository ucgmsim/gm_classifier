...
@pytest.mark.parametrize(
    ["model_dir", "model_name", "input_data_ffp"],
    [
        (
            os.path.join(original_models_dir, "Canterbury/model"),
            "canterbury",
            os.path.join(file_dir, "bench_canterbury_data.csv"),
        ), (
            os.path.join(original_models_dir, "CanterburyWellington/model"),
            "canterbury_wellington",
            os.path.join(file_dir, "bench_canterbury_wellington_data.csv"),
        )
    ],
)
def test_orig_predict(model_dir: str, model_name: str, input_data_ffp: str):
    # Contains both the input data and the expected output
    bench_df = pd.read_csv(input_data_ffp)
    result = gm.predict.run_original(
        model_dir, model_name, bench_df.loc[:, gm.features.FEATURE_NAMES]
    )
