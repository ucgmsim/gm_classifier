...
def main(features_dir: Path, model_dir: Path, output_ffp: Path, n_preds: int = 25):
    ...
    # Gathers a list of sub-directories (usually yearly). This is particularly useful
    # when a large database was broken up into smaller jobs.
    
    sub_files = features_dir.glob('*')
    for sub_file in sub_files:
        csv_files = sub_file.glob('*.csv')
        # Check if subdirectory contains features CSV files, else continue
        if not any(["features_comp_X.csv" in str(cur_file) for cur_file in csv_files]):
            continue
        print(sub_file)
        ...
        feature_df = gmc.utils.load_features_from_dir(sub_file, concat=True)
        ...
        results = pd.concat([results,result_df]) # Concatenate results
    
    if not os.path.exists(os.path.dirname(output_ffp)):
        os.makedirs(os.path.dirname(output_ffp))
    results.reset_index(drop=True)
    results.to_csv(output_ffp, index_label="record_id")
