...
# --------------- Build & compile model ------------------
gmc_model = gmc.model.build_score_simple_model(
    {
        **hyperparams,
        **{
            "hidden_layer_config": {"dropout": hyperparams["dropout"]},
            "out_act_fn": gmc.model.get_act_fn(
                hyperparams["out_act_f"],
...
gmc.plots.plot_score_true_vs_est(label_df.loc[train_ids], y_est_train, output_dir, title="training")
gmc.plots.plot_score_true_vs_est(label_df.loc[val_ids], y_est_val, output_dir, title="validation")
